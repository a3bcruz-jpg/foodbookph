...
export default async function CustomerProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const canSeeActivity = user.privacy.showActivity && (user.privacy.profileVisibility === "PUBLIC" || friendship);
  const [friends, reviews, posts] = await Promise.all([
    prisma.friendship.count({ where: { OR: [{ userAId: user.id }, { userBId: user.id }] } }),
    canSeeActivity ? prisma.review.findMany({ where: { userId: user.id }, orderBy: { createdAt: "desc" }, take: 6, select: { id: true, rating: true, body: true, createdAt: true, restaurant: { select: { name: true } } } }) : [],
    canSeeActivity ? prisma.post.findMany({ where: { authorId: user.id }, orderBy: { createdAt: "desc" }, take: 6, select: { id: true, caption: true, createdAt: true, media: { take: 1, select: { url: true, alt: true } } } }) : [],
  ]);
  ...
  return <main style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px 80px" }}>
    ...
    <section style={{ background: "#fff", border: "1px solid #e6eae5", borderRadius: 8, padding: 30 }}>
      ...
      {!user.privacy.showActivity ? <p style={{ color: "#77827e" }}>This customer has limited their food activity visibility.</p> : !canSeeActivity ? <p style={{ color: "#77827e" }}>This customer shares activity with connections only.</p> : <div style={{ display: "grid", gap: 12 }}>{reviews.map((review) => <article key={review.id} style={{ border: "1px solid #e6eae5", borderRadius: 6, padding: 16 }}><strong>{review.restaurant.name}</strong><span style={{ color: "#c98f22", marginLeft: 10 }}>{"★".repeat(Math.max(0, Math.min(5, review.rating)))}</span><p style={{ color: "#53645c", lineHeight: 1.5, marginBottom: 0 }}>{review.body}</p></article>)}{posts.map((post) => <article key={post.id} style={{ border: "1px solid #e6eae5", borderRadius: 6, padding: 16 }}><strong>FoodBookPH post</strong><p style={{ color: "#53645c", lineHeight: 1.5, marginBottom: 0 }}>{post.caption}</p></article>)}{!reviews.length && !posts.length ? <p style={{ color: "#77827e" }}>No public food activity yet.</p> : null}</div>}
    </section>
  </main>;
}
