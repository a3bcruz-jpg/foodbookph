...
export async function POST(request: Request) {
  ...
  if (!isOwner && !isAdmin) return NextResponse.json({ error: "Restaurant owner access required." }, { status: 403 });
    ...
    const response = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        const restaurant = await prisma.restaurant.findFirst({
          where: {
            id: restaurantId,
            ...(isAdmin ? {} : { memberships: { some: { userId: account.id, role: { in: ["OWNER", "MANAGER"] } } } }),
          },
          select: { id: true },
        });
        if (!restaurant) throw new Error("Restaurant not found or not managed by you.");
        if (!pathname.startsWith(`restaurants/${restaurantId}/posts/`)) throw new Error("Invalid upload path.");
        return {
