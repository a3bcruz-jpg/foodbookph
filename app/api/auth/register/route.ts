...
export async function POST(request: Request) {
  try {
    ...
    if (role !== "CUSTOMER" && role !== "RESTAURANT_OWNER") return NextResponse.json({ error: "Choose whether you are a Customer or a Restaurant Owner." }, { status: 400 });
    const account = await registerAccount({ email, password, username, displayName, role });
