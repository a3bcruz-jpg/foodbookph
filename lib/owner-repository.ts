...
export async function getOwnerDashboard(userId: string) {
  ...
  return {
    restaurant: {
      ...
      posts: restaurant.posts.map((post) => ({
        id: post.id,
...
export async function createOwnedRestaurant(
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    if (existingMembership) {
      throw new Error("You already have a restaurant connected to this account.");
    }
    ...
    await transaction.restaurantMembership.create({
      data: {
...
export async function respondToReview(
  ...
  return prisma.reviewResponse.upsert({
    where: { reviewId },
    update: { body },
    create: { reviewId, ownerId: userId, body },
  });
}
...
export async function createMenuCategory(
  ...
  const prisma = getPrisma();
  if (!prisma || !(await ownedRestaurant(userId, input.restaurantId))) {
    return null;
  }
...
export async function updateMenuCategory(
  ...
  const category = await prisma.menu.findFirst({
    where: {
      id: categoryId,
      restaurant: {
        memberships: {
...
export async function deleteMenuCategory(userId: string, categoryId: string) {
  ...
  if (category._count.items > 0) {
    return { deleted: false, hasItems: true };
  }
...
export async function updateOwnerPost(
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    return transaction.post.update({
      ...
      data: {
        caption,
        media: image ? { create: { url: image } } : undefined,
      },
      include: { media: { take: 1 } },
    });
  });
