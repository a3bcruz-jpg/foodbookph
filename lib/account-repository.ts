...
import { authenticate, createAccount, createSession, deleteSession, getAccountFromSession, publicAccount, updateAccount, updatePassword } from "@/lib/account-store";
...
export async function registerAccount(input: { email: string; password: string; username: string; displayName: string }) {
  ...
  const user = await prisma.user.create({ data: { email: input.email.trim().toLowerCase(), username: input.username.trim().toLowerCase().replace(/^@/, ""), displayName: input.displayName.trim(), passwordHash: (await import("@/lib/account-store")).hashPassword(input.password), roles: { create: { role: "CUSTOMER" } } }, include: { roles: true } });
